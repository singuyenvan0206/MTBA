    ...
    reader.onload = async (evt) => {
      try {
        ...
        for (let i = 1; i < lines.length; i++) {
          ...
          // Check for duplicate email and phone client-side
          const isDuplicateEmail = users.some((u: any) => u.email?.trim().toLowerCase() === email.trim().toLowerCase());
          const isDuplicatePhone = phone && users.some((u: any) => u.phone?.trim() === phone.trim());
          ...
          if (isDuplicatePhone) {
            errorCount++;
            errors.push(`Dòng ${i + 1}: Số điện thoại "${phone}" đã tồn tại trong hệ thống`);
            continue;
          }
